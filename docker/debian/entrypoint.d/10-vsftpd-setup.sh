  ...
  if [ "$FTP_MODE" = "anonymous" ]; then
    ...
    if [ -n "$FTP_USER" ] && [ -n "$FTP_PASS" ]; then
      if id -u "$FTP_USER" >/dev/null 2>&1; then
        ...
        printf '%s\n%s\n' "$FTP_PASS" "$FTP_PASS" | adduser -h /home/"$FTP_USER" -s /sbin/nologin "$FTP_USER" >/dev/null 2>&1
      fi
    fi
    ...
    if [ -n "$FTP_ROOT_PWD" ]; then
      ...
      HASHED_FTP_ROOT_PWD=$(printf '%s\n' "$FTP_ROOT_PWD" | openssl passwd -1 -stdin)
    ...
    if [ -n "$FTP_USER" ] && [ -n "$FTP_PASS" ]; then
      ...
      HASHED_FTP_USER_PASSWORD=$(printf '%s\n' "$FTP_PASS" | openssl passwd -1 -stdin)
